...
def evaluate_model(model, X, y_true, label_names=["Home Win","Draw","Away Win"]):
    ...
    try:
        if n_cls == 2:
            ...
            m["roc_auc_ovr"] = round(roc_auc_score(y_onehot, y_proba,
...
def backtest(features_df, model, feature_names, test_seasons=None,
    ...
    for col in feature_names:
        if col not in df.columns: df[col] = -999.0
    X      = df[feature_names].astype(np.float32).fillna(-999)
    probas = model.predict_proba(X)

    bankroll, bet_log, bh = starting_bankroll, [], [starting_bankroll]
    ...
    for i, (idx, row) in enumerate(df.iterrows()):
        ...
        bh.append(bankroll)
        bet_log.append({"date":row.get("date"),"league":row.get("league_key"),
    ...
    if bet_df.empty:
        ...
        return {"n_bets":0,"roi":0.0,"profit_units":0.0,"bankroll_history":bh,"bet_log":bet_df}
    ...
    return {"n_bets":len(bet_df), "win_rate":round(bet_df["won"].mean(),4),
...
if __name__ == "__main__":
    ...
    features_path = PATHS.MODELS / "match_result_features.pkl"
    if not model_path.exists():
        logger.error(f"Model not found: {model_path}. Run: python src/train.py")
        sys.exit(1)
    ...
    if feature_names is None:
        logger.error("Feature list not found. Re-run python src/train.py")
        sys.exit(1)
    ...
    X_test = test_df[feature_names].astype(np.float32).fillna(-999)
